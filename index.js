...
app.post('/all-consultations', (req, res) => {
    ...
    Conversationid.findOne({ conversationid: received.conversation.id }, (err, obj) => {
        if (obj) {
            ...
            Consultations.find({ studentID: studentid, isDone: false, isApprovedByStudent: true, isApprovedByTeacher: true, startDate: { $gte: ISODate(new Date()) } }).sort({ 'startDate.$date': -1 }).limit(5).exec((err2, consultations) => {
